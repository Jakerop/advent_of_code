...
def check_top(trees, tree_height, trees_seen, x_pos, y_pos):
    if y_pos == 0:
        return [True, trees_seen]
    elif tree_height <= trees[y_pos-1][x_pos]:
        return [False, trees_seen+1]
    else:
        return check_top(trees, tree_height, trees_seen+1, x_pos, y_pos-1)

def check_bottom(trees, tree_height, trees_seen, x_pos, y_pos):
    if y_pos == len(trees)-1:
        return [True, trees_seen]
    elif tree_height <= trees[y_pos+1][x_pos]:
        return [False, trees_seen+1]
    else:
        return check_bottom(trees, tree_height, trees_seen+1, x_pos, y_pos+1)

def check_left(trees, tree_height, trees_seen, x_pos, y_pos):
    if x_pos == 0:
        return [True, trees_seen]
    elif tree_height <= trees[y_pos][x_pos-1]:
        return [False, trees_seen+1]
    else:
        return check_left(trees, tree_height, trees_seen+1, x_pos-1, y_pos)

def check_right(trees, tree_height, trees_seen, x_pos, y_pos):
    if x_pos == len(trees[y_pos])-1:
        return [True, trees_seen]
    elif tree_height <= trees[y_pos][x_pos+1]:
        return [False, trees_seen+1]
    else:
        return check_right(trees, tree_height, trees_seen+1, x_pos+1, y_pos)
...
if __name__ == "__main__":
    ...
    print(f"best scenic score: {best_scenic_score}, best_tree = {best_tree}")
